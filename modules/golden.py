...
def run(
    ...
    # ── گروه‌بندی و محاسبه معیارهای خام ──────────────────────────────────
    log.info("محاسبه معیارهای خام...")
    raw_records = []
    groups = sig_df.groupby(["strategy_folder", "coin_composition", "signature"])
    total_groups = groups.ngroups
    log.info(f"تعداد کل گروه‌ها: {total_groups:,}")
